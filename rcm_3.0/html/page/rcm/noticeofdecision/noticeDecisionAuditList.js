ctmApp.register.controller('NoticeDecisionAuditList', ['$http','$routeParams','$scope','$location', function ($http,$routeParams,$scope,$location) {
    $scope.orderby='desc';
    ...
    $scope.queryApplyList=function(){
    	$http({
			method:'post',  
		    url: srvUrl + "noticeDecisionAudit/queryWaitList.do",
    ...
    $scope.queryApplyedList=function(){
    	$http({
			method:'post',  
		    url: srvUrl + "noticeDecisionAudit/queryAuditedList.do",
...
//		    data: $.param({"UUID":JSON.stringify($scope.credentials.UUID)})
//		}).success(function(result){
//			if(data.result_code=="S"){
//                var file=data.result_data.filePath;
//                var fileName=data.result_data.fileName;

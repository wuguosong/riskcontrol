ctmApp.register.controller('NoticeDecisionDraftCompleteDetail', ['$http', '$scope', '$location', '$routeParams', 'Upload', '$timeout', function ($http, $scope, $location, $routeParams, Upload, $timeout) {
    ...
    $scope.complete = function (tag) {
        if ($("#noticeName").valid()) {
            $scope.nod.projectFormalId = $scope.businessid;
    ...
    $scope.getNoticeDecstionByID = function (id) {
        ...
        $http({
            ...
            console.log(noticeDecision);
            if (noticeDecision == null) {
                //新增时查询项目信息
                //查询项目基本信息
                $http({
                    method: 'post',
                    ...
                    $scope.requireTask2 = '';
                    $scope.requireTask3 = '';
                    noticeDecision.projectFormalId = $scope.pfr._id;
                    noticeDecision.projectName = $scope.pfr.apply.projectName;
                    noticeDecision.reportingUnit = $scope.pfr.apply.reportingUnit;
                    noticeDecision.pertainArea = $scope.pfr.apply.pertainArea;
                    noticeDecision.projectNo = $scope.pfr.apply.projectNo;
                    noticeDecision.controllerVal = "NoticeDecision";
                    noticeDecision.createBy = {name: $scope.credentials.userName, value: $scope.credentials.UUID};

ctmApp.register.controller('ManagerFeedback',['$http','$scope','$location','$routeParams','Upload','$timeout', function ($http,$scope,$location,$routeParams,Upload,$timeout) {
    //初始化
    ...
    $scope.saveFormalReviewComtents=function(openPopWin){
        ...
        var comlistdate = $scope.pfr.approveAttachment.commentsList;
        if(typeof(openPopWin)=='function') {
            if (null != comlistdate) {
                for (var i = 0; i < comlistdate.length; i++) {
                    var va = comlistdate[i].commentDate;
                    if (va == '' || undefined==va) {
                        $scope.pfr.approveAttachment.commentsList[i].commentDate = myDate.getFullYear() + "-" + paddNum(myDate.getMonth() + 1) + "-" + myDate.getDate();
                    }
                    var feedbackTaskID = comlistdate[i].feedbackTaskID;
                    if (feedbackTaskID == '' || undefined==feedbackTaskID) {
                        $scope.pfr.approveAttachment.commentsList[i].feedbackTaskID=$scope.taskID;
                    }
                }
            }
        }
...
// textArea换行符转<br/>
    ...
    $scope.showSubmitModal=function(){
        $scope.saveFormalReviewComtents(function(){

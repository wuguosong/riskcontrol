ctmApp.register.controller('FormalBiddingInfoReview', ['$http','$scope','$location','$routeParams','Upload',
 function ($http,$scope,$location,$routeParams,Upload) {
  ...
  //初始化页面所需数据
	 ...
	 $scope.getByID = function(projectFormalId){
	        $http({
			    ...
			    data: $.param({"projectFormalId":projectFormalId})
		         ...
		         $scope.meetFiles = data.result_data.Formal.meetingFiles;
		         $scope.meetInfo = data.result_data.MeetInfo;
		         $scope.applyDate = data.result_data.applyDate;
		         //处理附件列表
		         $scope.reduceAttachment(data.result_data.Formal.attachment,data.result_data.Formal.meetingFiles);
		         //新增附件类型
	...
	//处理附件列表
	    $scope.reduceAttachment = function(attachment,meetingFiles){
	    	$scope.newAttachment = [];
				for(var i in attachment){
		    		...
		    		if(files!=null && undefined!=files){
		    			...
		    			for(var j in files){
		    				...
		    				files[j].UUID=uuid;
		    				//只显示选中的附件
		    				if($scope.isValueExist(meetingFiles, 'UUID', files[j].UUID))
		    				{

...
//		params.put("orderBy", orderBy);
	...
	private Map<String, Object> packageDataForOracle_V01(Document doc) {
		...
		// serviceType
		List<JSONObject> serviceType = (List<JSONObject>) apply.get("serviceType");
		if (Util.isNotEmpty(serviceType) && serviceType.size() > 0) {
			params.put("serviceType", serviceType.get(0).get("KEY"));
		}
...
//				map.put("projectReportURL", docOld.get("filePath"));
	...
	public Document updateServiceType(Document doc) {
		...
		/**
		 * 1--1401 2--1402 4--1401 5--1404 6--1403 7--1405
		 */
	...
	@Override
	public void updateServiceTypeOpinionByBusinessId(String json, String businessId) {
	...
	public void saveLegalReviewInfo(String json, String businessId) {
		...
		// {"_id",ObjectId("adfa-sf51-5181-811s-85ad")}
	...
	public void saveFixGroupOption(String json) {
		...
		Map<String, Object> formalAssessment = this.baseMongo.queryById(businessId,
	...
	@Override
	public Map<String, Object> queryAuditedByConditions(String userId, String businessId) {
		Map<String, Object> conditions = new HashMap<String, Object>();
		conditions.put("userId", userId);
		conditions.put("businessId", businessId);
...
//		params.put("orderBy", orderBy);
	...
	public void addConferenceInformation(String json) {
		...
		// 修改oracle的stage状态
		/*
		 * Map<String, Object> map = new HashMap<String, Object>(); //map.put("stage",
		 * "3.5"); // 数据所在目录修改 状态随之修改 map.put("stage", "3.9"); map.put("need_meeting",
		 * "1"); map.put("metting_commit_time", Util.getTime()); map.put("businessId",
		 * pfr.getString("formalId")); this.formalAssessmentInfoMapper.updateStage(map);
		 */
	...
	public Result saveOrUpdateForTz(Document doc, Result result) {
		...
		String prefix = PropertiesUtil.getProperty("domain.allow");
		map.put("URL", prefix + "/html/index.html#/FormalAssessmentDetailView/" + businessid + "/"
				+ Util.encodeUrl("#/FormalAssessmentInfoList/0"));
...
//		baseMongo.updateSetByObjectId(businessId, formalAssessmentMongo, RCM_FORMALASSESSMENT_INFO);
	...
	@Override
	public List<Map<String, Object>> queryFormalCount(String wf_state, String stage, String result,
	...
	public void updateAttachment(String json) {
		...
		if (Util.isNotEmpty(newItem) && newItem) {
			...
			this.saveFileLogs(businessId, (String) user.get("UUID"), (String) user.get("NAME"), objectId.toHexString(),
					fileName, filePath, "新增投资部门附件类型与附件");
			...
			for (Map<String, Object> attachment : attachmentList) {
				...
				if (uuid.equals(itemUUID)) {
					...
					this.saveFileLogs(businessId, (String) user.get("UUID"), (String) user.get("NAME"),
							objectId.toHexString(), fileName, filePath, "新增投资部门附件");
					files.add(newFile);
	...
	public Result updateRiskAttachment(String json) {
		...
		if (Util.isNotEmpty(newFile) && newFile) {
			...
			this.saveFileLogs(businessId, (String) user.get("UUID"), (String) user.get("NAME"), UUID, fileName,
					filePath, "新增风控附件");
		} else {
			// 替换附件
				...
				for (Map<String, Object> map : files) {
					...
					if (fUUID.equals(fileUUID)) {
						...
						if (!owner.get("VALUE").equals(user.get("UUID"))) {
							...
							this.saveFileLogs(businessId, (String) user.get("UUID"), (String) user.get("NAME"),
									fileUUID, fileName, filePath, "删除投资部门附件");
					...
					for (Map<String, Object> file : files) {
						...
						if (fileUuid.equals(UUID)) {
							...
							if (Util.isNotEmpty(fileRemark)) {
								...
								Map<String, Object> data = new HashMap<String, Object>();
								data.put("fileRemark", fileRemark);
								this.baseMongo.updateSetByObjectId(businessId, data,
										Constants.RCM_FORMALASSESSMENT_INFO);
							}
							...
							file.put("approved", owner);
							file.put("programmed", owner);
							this.saveFileLogs(businessId, (String) user.get("UUID"), (String) user.get("NAME"), UUID,
									fileName, filePath, "投资部门附件替换，替换源文件");
						}
					}
				} else {
					Map<String, Object> newFile = new HashMap<String, Object>();
	...
	public void saveFileLogs(String businessId, String userId, String userName, String fileUuid, String fileName,
			String filePath, String desc) {
		Map<String, Object> formalMongo = this.baseMongo.queryById(businessId, Constants.RCM_FORMALASSESSMENT_INFO);

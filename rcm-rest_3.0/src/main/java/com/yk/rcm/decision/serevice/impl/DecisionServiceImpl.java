...
public class DecisionServiceImpl implements IDecisionService {
	...
	private Logger logger = Logger.getLogger("DecisionServiceImpl");
	...
	@Override
	public void addDecisionOpinion(String formalId, String formalTypeStr, String aagreeOrDisagree) {
		synchronized (formalId) {
			...
			case 0:
				queryData = baseMongo
						.queryByCondition(new BasicDBObject("formalId", formalId), Constants.FORMAL_MEETING_INFO)
						.get(0);
	...
	public Map<String, Object> getDecisionResult(String id) {
		...
		meetingLeaders = meetingIssueService
				.queryMeetingLeadersByMeetingIssueId(resultData.get("MEETING_ISSUE_ID").toString());
		...
		synchronized (id) {
			...
			if (huiYiZhuXi == 2) {
				...
				// 走正常计算流程
				decisionResult = decisionCalculation(tongYiCount, tiaoJianTongYiCount, zongRenShu, buTongYiCount,
						huiYiZhuXi);
				...
				if (zongRenShu < 7 && zongRenShu < meetingLeaderCount
						&& tongYiCount + tiaoJianTongYiCount != zongRenShu) {
					resultData.put("isLiChangGanRaoQuan", 0);

					int differSize = meetingLeaderCount - zongRenShu;
					// 第1种情况，离场人员选择同意
					int tongYiCount2 = tongYiCount + differSize;
					int decisionResult1 = decisionCalculation(tongYiCount2, tiaoJianTongYiCount, meetingLeaders.size(),
							buTongYiCount, huiYiZhuXi);

					// 第2种情况，离场人员选择不同意
					int buTongYiCount2 = buTongYiCount + differSize;
					int decisionResult2 = decisionCalculation(tongYiCount, tiaoJianTongYiCount, meetingLeaders.size(),
							buTongYiCount2, huiYiZhuXi);
			...
			if (null == decisionResultText || "0".equals(decisionResultText.toString())) {
				...
				case 0:
					...
					// 给投资系统推送信息
					TzAfterNoticeClient tzAfterNoticeClient = new TzAfterNoticeClient(formalId, decisionResult + "",
							null);
				...
				case 2:
					...
					TzAfterPreReviewClient tzAfterPreReviewClient = new TzAfterPreReviewClient(formalId,
							decisionResult + "", null);
	 ...
	 * 根据规则计算决策结果
	 */
	private int decisionCalculation(int tongYiCount, int tiaoJianTongYiCount, int zongRenShu, int buTongYiCount,
			int huiYiZhuXi) {
		int decisionResult;
	 ...
	 * 根据规则计算决策结果
	 */
	private int decisionCalculationNew(int tongYiCount, int tiaoJianTongYiCount, int zongRenShu, int buTongYiCount,
			int huiYiZhuXi) {
		int decisionResult;
		// 如果“同意投资与同意有条件投资”表决票数合计超过三分之二
		if (isDaYuFanMu(tongYiCount + tiaoJianTongYiCount, zongRenShu, 3, 2)) {
	...
	public void updateMeetingLeadersById(String id, List<Map<String, Object>> meetingLeaders, String chairmanId) {
		...
		if ("0".equals(formalType)) {
			...
			this.baseMongo.updateSetByFilter(new BasicDBObject("formalId", businessId), data,
	...
	public boolean isTodayDecision() {
		// 页面js判断
		// boolean ifUserIsJueCe =
		// roleService.ifRoleContainUser(Constants.ROLE_CODE_DECISION_LEADERS);
		// if(!ifUserIsJueCe){
		// return false;
		// }
	...
	@Override
	public void addDecisionOpinionNew(String formalId, String formalTypeStr, String aagreeOrDisagree,
			String zhuxiStatus) {
		synchronized (formalId) {
			// 获取当前用户
			String userId = ThreadLocalUtil.getUserId();
			...
			case 0:
				...
				params.put("businessId", formalId);
				String Json = JSON.toJSONString(params);
				try {
					reportDataService.saveOrUpdateReportData(Json);
					logger.info("同步报表数据成功：[" + params.get("projectType") + "," + formalId + "]");
				} catch (Exception e) {
					logger.info("同步报表数据出错：[" + params.get("projectType") + "," + formalId + "],错误详情：" + e.getMessage());
				}
			...
			case 1:
				...
				params1.put("projectType", "bulletin");
				params1.put("businessId", formalId);
				String JsonB = JSON.toJSONString(params1);
				try {
					reportDataService.saveOrUpdateReportData(JsonB);
					logger.info("同步报表数据成功：[" + params1.get("projectType") + "," + formalId + "]");
				} catch (Exception e) {
					logger.info(
							"同步报表数据出错：[" + params1.get("projectType") + "," + formalId + "],错误详情：" + e.getMessage());
				}
			...
			case 2:
				...
				params2.put("projectType", "pre");
				params2.put("businessId", formalId);
				String JsonP = JSON.toJSONString(params2);
				try {
					reportDataService.saveOrUpdateReportData(JsonP);
					logger.info("同步报表数据成功：[" + params2.get("projectType") + "," + formalId + "]");
				} catch (Exception e) {
					logger.info(
							"同步报表数据出错：[" + params2.get("projectType") + "," + formalId + "],错误详情：" + e.getMessage());
					e.printStackTrace();
				}
			...
			if (zhuxiStatus == 1) {
				...
				if (huiYiZhuXi == 2) {
					...
					if (zongRenShu < 7 && zongRenShu < meetingLeaderCount) {
						...
						// 第1种情况，离场人员选择同意
						int tongYiCount2 = tongYiCount + differSize;
						int decisionResult1 = decisionCalculation(tongYiCount2, tiaoJianTongYiCount,
								meetingLeaders.size(), buTongYiCount, huiYiZhuXi);

						// 第2种情况，离场人员选择不同意
						int buTongYiCount2 = buTongYiCount + differSize;
						int decisionResult2 = decisionCalculation(tongYiCount, tiaoJianTongYiCount,
								meetingLeaders.size(), buTongYiCount2, huiYiZhuXi);
				...
				// 走正常计算流程,主席未行使一票否决权
				decisionResult = decisionCalculationNew(tongYiCount, tiaoJianTongYiCount, zongRenShu, buTongYiCount,
						huiYiZhuXi);
				// 表决人数小于7个，考虑离场
				// 表决人数少于会议安排人数(5+1)，考虑离场
				// 如果有条件同意票数 等于 表决人数，则不考虑离场人员
				if (zongRenShu < 7 && zongRenShu < meetingLeaderCount) {
					...
					// 第1种情况，离场人员选择同意
					int tongYiCount2 = tongYiCount + differSize;
					int decisionResult1 = decisionCalculationNew(tongYiCount2, tiaoJianTongYiCount,
							meetingLeaders.size(), buTongYiCount, huiYiZhuXi);

					// 第2种情况，离场人员选择不同意
					int buTongYiCount2 = buTongYiCount + differSize;
					int decisionResult2 = decisionCalculationNew(tongYiCount, tiaoJianTongYiCount,
							meetingLeaders.size(), buTongYiCount2, huiYiZhuXi);

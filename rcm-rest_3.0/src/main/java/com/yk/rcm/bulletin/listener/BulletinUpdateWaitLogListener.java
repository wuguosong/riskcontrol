 ...
 */
@Component
public class BulletinUpdateWaitLogListener implements TaskListener {
	...
	public void notify(DelegateTask delegateTask) {
		...
		boolean isExistWaitLog = this.bulletinAuditLogService.isExistWaitLog(map);
		if(isExistWaitLog){
	...
	private void updateExist(DelegateTask delegateTask, String userId, String taskId, String executionId){
		...
		data.put("orderBy", this.bulletinAuditLogService.queryNextOrderNum(businessId));
		data.put("taskdesc", taskdesc);
		data.put("taskId", taskId);
		data.put("executionId", executionId);
		this.bulletinAuditLogService.updateWaitLog(data);
	}

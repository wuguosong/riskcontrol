...
//			Date object = (Date) paramsForOracle.get("create_date");
	...
	public Map<String, Object> queryNoticeDecstionByFormalId(String formalId) {
		...
		Map<String,Object> oracle = noticeDecisionDraftInfoMapper.queryById(id);
	...
	public Map<String, String> getNoticeOfDecisionWord(String businessId) {
		...
		param.put("decisionNo",String.format("JCTZ－%tY-%s", new Date(),Util.isEmpty(data.get("decisionNo"))?"":data.get("decisionNo")));
		...
		//评审总投资-类型
		boolean reviOfTotaInveTypeIsNull = null == data.get("reviOfTotaInveType") || "".equals(data.get("reviOfTotaInveType"));
		param.put("reviOfTotaInveType",reviOfTotaInveTypeIsNull?"":"(".concat(data.get("reviOfTotaInveType").toString()).concat(")"));
		...
		String implMatt = "";
		if(null == implementationMatters || StringUtils.isEmpty(implementationMatters.toString()) || "无".equals(implementationMatters.toString().trim())){

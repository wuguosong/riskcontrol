 ...
 *
 */
@Service("refreshPrfPertainArea")
@Transactional
public class RefreshPrfPertainArea implements Initable {
	...
	private void mongdb() {
		...
		documents.forEach(new Block<Document>(){
			...
			public void apply(Document document) {
				...
				baseMongo.updateSetByObjectId(objectId.toString(), updateDocument, Constants.RCM_FORMALASSESSMENT_INFO);
			}
		});		
	}

	private void oracle() {
		List<Map<String,Object>> pfrs = formalAssessmentInfoService.queryPertainAreaIsNull();
		for (Map<String, Object> pfr : pfrs) {
			...
			formalAssessmentInfoService.updatePertainAreaId(pfr.get("ID").toString(), pertainAreaId);
		}		
	}
}

...
public class BreakpointController {
     ...
     * @return String
     */
    @RequestMapping(value = "v2/breakpoint/progress", method = RequestMethod.POST)
    @ResponseBody
    public String progress2(String fileName) {
        logger.info("v2/breakpoint/progress2<<");
        String progressLine = CacheUtil.get("progressLine_" + fileName);
        if (StringUtils.isBlank(progressLine)) {
            progressLine = "0";
        }
        logger.info("fileName:" + fileName + " progress:" + progressLine);
     ...
     * @return Map<String, Object>
     */
    @RequestMapping(value = "v2/breakpoint/check", method = RequestMethod.POST)
    @ResponseBody
    public Map<String, Object> check2(String fileName, String progressLine, String fileMd5, String chunk, String chunkSize) {
        logger.info("v2/breakpoint/check2<<");
        String savePath = PathKit.getWebRootDisk() + "/upload";
        Map<String, Object> json = new HashMap<String, Object>();
        String lastUploadTime = DateUtil.getDateToString(new Date(), DateUtil.DATEFORMAT_YYYY_MM_DD_HH_MM_SS);
        String fileName_ = CacheUtil.get("fileName_" + fileName);
        if (StringUtils.isBlank(fileName_)) {
            // 存储文件唯一MD5标识,格式{"fileName_<fileName>":<fileMd5>}
            CacheUtil.set("fileName_" + fileName, fileMd5);
        }
        CacheUtil.set("lastUploadTime_" + fileName, lastUploadTime);
        CacheUtil.set("progressLine_" + fileName, progressLine);
        // 获取分区文件
        File checkFile = new File(savePath + "/" + CacheUtil.get("fileName_" + fileName) + "/" + chunk);
        // 判断该分区文件是否存在且完整
        if (checkFile.exists() && checkFile.length() == Integer.parseInt(chunkSize)) {
            json.put("exist", true);
        } else {
     ...
     * @return Map<String, Object>
     */
    @RequestMapping(value = "v2/breakpoint/merge", method = RequestMethod.POST)
    @ResponseBody
    public Map<String, Object> merge2(String fileName, String fileMd5) {
        Map<String, Object> json = new HashMap<String, Object>();
        logger.info("v2/breakpoint/merge2<<进入");
        logger.info("参数信息:" + "{fileName:" + fileName + ",fileMd5:" + fileMd5 + "}");
        String basePath = PathKit.getWebRootDisk() + "/upload";
        // 合并文件
        try {
            // 读取目录里的所有文件
            File f = new File(basePath + "/" + CacheUtil.get("fileName_" + fileName));
            // 获取分片数组
            File[] fileArray = f.listFiles();
            // 转成集合，便于排序
            List<File> fileList = new ArrayList<File>(Arrays.asList(fileArray));
            ...
            // 清除文件夹
            File tempFile = new File(basePath + "/" + CacheUtil.get("fileName_" + fileName));
            if (tempFile.exists() && tempFile.isDirectory()) {
                tempFile.delete();
            }
            // 将文件的最后上传时间和生成的文件名返回
            json.put("lastUploadTime", CacheUtil.get("lastUploadTime_" + fileName));
            json.put("pathFileName", CacheUtil.get("fileName_" + fileName) + suffix);
            // 清除文件相关缓存信息
            CacheUtil.remove("progressLine_" + fileName);
            CacheUtil.remove("lastUploadTime_" + fileName);
            CacheUtil.remove("fileName_" + fileName);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error(e.getMessage());
        }
        logger.info("v2/breakpoint/merge2>>退出");
        return json;
    }
     ...
     * @param size             文件大小
     */
    @RequestMapping(value = "v2/breakpoint/save", method = RequestMethod.POST)
    @ResponseBody
    public String save2(HttpServletRequest request, String id, String fileMd5, String chunk, String name, String type, String lastModifiedDate, String size) {
        logger.info("v2/breakpoint/save2<<进入");
        logger.info("************FormData*************");
        logger.info("id:" + id);
        logger.info("fileMd5:" + fileMd5);
        logger.info("chunk:" + chunk);
            ...
            if (multipartResolver.isMultipart(request)) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (multipartFile != null) {
                        String rootDisk = PathKit.getWebRootDisk();
                        String fileName = multipartFile.getOriginalFilename();
                        String tempPath = "upload" + "/" + CacheUtil.get("fileName_" + name);
                        String tempDiskPath = rootDisk + "/" + tempPath + "/";
                        File file = new File(tempDiskPath);
                        if (!file.exists()) {
                            file.mkdirs();
        ...
        logger.info("v2/breakpoint/save2>>退出");
        return fileMd5;
    }
}

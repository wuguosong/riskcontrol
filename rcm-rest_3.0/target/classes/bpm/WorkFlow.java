 ...
 * @Description: 流程处理通用类
 * @Author zhangkewei
 * @Date 2016年8月23日 下午7:06:03  
 */
public class WorkFlow extends BaseService{
	...
	public WorkFlow(){
		e = BpmFactory.getInstance();
		runtimeService = e.getRuntimeService();
		repositoryService = e.getRepositoryService();
		taskService = e.getTaskService();
	}
	
	//部署流程
	public void deploy(String json) throws Exception {
		Map<String, Object> params = Util.parseJson2Map(json);
		String processKey = (String)params.get("processKey");
		if(Util.isEmpty(processKey))return;
		try {
			// 获取流程引擎
			// 获取仓库服务的实例
			Deployment deployment = e.getRepositoryService()//
			.createDeployment()//
			.addClasspathResource("diagrams/"+processKey+".bpmn")
			.addClasspathResource("diagrams/"+processKey+".png")
			.name(processKey)
			.deploy();
			//保存urserTask信息
			ProcessDefinition pd = e.getRepositoryService().createProcessDefinitionQuery().deploymentId(deployment.getId()).singleResult();
			ProcessDefinitionEntity pde = (ProcessDefinitionEntity) ((RepositoryServiceImpl)e.getRepositoryService()).
					getDeployedProcessDefinition(pd.getId());
			List<ActivityImpl> defActs = pde.getActivities();
			TaskWarningConf tw = new TaskWarningConf();
			...
			for(ActivityImpl act : defActs){
				if("subProcess".equals(act.getProperty("type"))){
					...
					for(ActivityImpl subAct : subActs){
						tw.addTaskWarningInfo(retList, subAct, pd.getId());
					}
				}else{
					tw.addTaskWarningInfo(retList, act, pd.getId());
				}
			}
			tw.insert(retList);
		} catch (Exception e) {
...
//		sendNotice(map, processInstanceId);
		...
		if(cesuanFileOpinion != null){
			...
			if(Constants.PRE_ASSESSMENT.equals(def.getKey())){
				...
				//正式评审
				IBaseMongo preAssementService = (IBaseMongo) SpringUtil.getBean("baseMongo");
				preAssementService.updateSetByObjectId(businessId, data, Constants.RCM_FORMALASSESSMENT_INFO);
			}
		}
...
//		pc.start();
	...
	public Document getProcessDefinitionId(String json){
		...
		if(StringUtils.isNotBlank(processInstanceId)){
			...
			if (businessId == null) {
				List<ProcessDefinition> list = e.getRepositoryService()
						.createProcessDefinitionQuery()
						.processDefinitionKey(processDefinitionKey)
						.orderByProcessDefinitionVersion().desc()
						.listPage(0, 1);
				if (Util.isEmpty(list)){
					throw new BusinessException("流程未部署:" + processDefinitionKey);
				}
				processDefinitionId = list.get(0).getId();
			}else{
				ProcessInstance pi = e.getRuntimeService().createProcessInstanceQuery()
						.processInstanceBusinessKey(businessId, processDefinitionKey)
						.singleResult();
				if(pi == null){
					List<HistoricProcessInstance> hli = e.getHistoryService().createHistoricProcessInstanceQuery()
						.processDefinitionKey(processDefinitionKey)
						.processInstanceBusinessKey(businessId)
						.list();
					if(hli.size() > 0){
						processDefinitionId = hli.get(0).getProcessDefinitionId();
					}else{
						List<ProcessDefinition> list = e.getRepositoryService()
								.createProcessDefinitionQuery()
								.processDefinitionKey(processDefinitionKey)
	...
	private void addCurrentAct(JsonArray array, ActivityImpl act) {
	 ...
	 //已办
    public PageAssistant getMyCompletedTask(String json){
		PageAssistant assistant = new PageAssistant(json);
		this.selectByPage(assistant, "workflow.selectMyCompleteTask");
		return assistant;
	} 
    
	//查询待办任务
	public PageAssistant getMyTask(String json){
		PageAssistant assistant = new PageAssistant(json);
		this.selectByPage(assistant, "workflow.selectToDoTask");
		return assistant;
	}
    ...
    private void sendNotice(Task task){
		...
		BpmnModel bpmnModel = this.repositoryService.getBpmnModel(pd.getId());
		org.activiti.bpmn.model.Process p = bpmnModel.getMainProcess();
		Collection<FlowElement> elements = p.getFlowElements();
		
		for(FlowElement element : elements){
			//遍历所有节点
			if(element instanceof UserTask && element.getId().equals(task.getTaskDefinitionKey())){
				//找到当前节点
				List<ActivitiListener> listeners = ((UserTask) element).getTaskListeners();
				for(ActivitiListener listener : listeners){
					//遍历所有的taskListener,判断有没有发待办的listener
					String delegate = listener.getImplementation();
					if("${projectWaitListener}".equals(delegate)){

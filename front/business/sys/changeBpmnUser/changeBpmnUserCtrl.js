define(['app', 'Service'], function (app) {
    app
        .register.controller('changeBpmnUserCtrl', ['$http', '$scope', '$location', '$stateParams', 'BEWG_URL', 'Window',
        function ($http, $scope, $location, $stateParams, BEWG_URL, Window) {
            console.log('changeBpmnUser');
            ...
            $scope.getFormalAssessmentByID=function(id){
                $http({
                    method:'post',
                    url:BEWG_URL.SelectPfrByIdChangeBpmnUser,
                    data: $.param({"id":id})
                }).success(function(data){
            ...
            $scope.getPreById = function(businessId){
                $http({
                    method:'post',
                    url:BEWG_URL.SelectPreByIdChangeBpmnUser,
            ...
            $scope.preAuditLogs = function (businessId){
                $http({
                    method:'post',
                    url: BEWG_URL.SelectPreAuditLogsByIdChangeBpmnUser,
                    data: $.param({"businessId":businessId})
            ...
            $scope.pfrAuditLogs = function (businessId){
                $http({
                    method:'post',
                    url: BEWG_URL.SelectPfrAuditLogsByIdChangeBpmnUser,
                    data: $.param({"businessId":businessId})
                }).success(function(result){
                    $scope.auditLogs = result.result_data;
                });
            ...
            $scope.bulletinAuditLogs = function(businessId){
                $http({
                    method:'post',
                    url: BEWG_URL.SelectBulletinAuditLogsByIdChangeBpmnUser,
                    data: $.param({"businessId": businessId})
                }).success(function(result){
            ...
            $scope.changeAuditUser = function(){
                ...
                if($scope.flow.task == null || $scope.flow.task == '' || $scope.flow.user == ''){
                    Window.alert("请选择需要替换的节点！");
                    return;
                }
                if($scope.flow.user.VALUE == null || $scope.flow.user.VALUE == ''){
                    Window.alert("请选择替换人员！");
                    return;
                }

                $http({
                    method:'post',
                    url:BEWG_URL.UpdateAuditUser,
                    data: $.param({"flow":JSON.stringify($scope.flow)})
                }).success(function(data){
                    if(data.success){

define(['app', 'Service'], function (app) {
    app
        .register.controller('noticeDecisionDraftListCtrl', ['$http', '$scope', '$location', '$stateParams', 'BEWG_URL', 'Window',
        function ($http, $scope, $location, $stateParams, BEWG_URL, Window) {
            console.log('noticeDecisionDraftList');
...
//    $scope.paginationConfMeeting.queryObj = {userId:$scope.credentials.UUID};
            ...
            $scope.queryApplyList=function(){
                $http({
                    method:'post',
                    url: SRV_URL + "noticeDecisionDraftInfo/queryApplyList.do",
            ...
            $scope.notMeetingList=function(){
                $http({
                    method:'post',
                    url: SRV_URL + "noticeDecisionDraftInfo/notMeetingList.do",
            ...
            $scope.queryApplyedList=function(){
                ...
                $http({
                    method:'post',
                    url: SRV_URL + "noticeDecisionDraftInfo/queryApplyedList.do",
            ...
            $scope.Delete = function () {
                ...
                $http({
                    method:'post',
                    url: SRV_URL + "noticeDecisionDraftInfo/delete.do",
            ...
            $scope.update = function () {
                var chks = $("input[type=checkbox][name=chk_start]:checked");
                if(chks.length != 1){
                    Window.alert("请选择其中一条数据编辑！");
                    return;
                }
                var uid = chks.get(0).value;
                $location.path("/index/NoticeDecisionDraftInfo/Update/" + uid);
            }
            //完成
            $scope.complete = function () {
                Window.alert("该功能仅当选中项目已经过会才能使用，请确认选中的项目已经过会！");
                var chks = $("input[type=checkbox][name=chk_start]:checked");
                if(chks.length != 1){
                    Window.alert("请选择其中一条数据编辑！");
                    return;
               ...
               /* var returnUrl = $filter("encodeURI")("#/NoticeDecisionDraftList");*/
                $location.path("/index/NoticeDecisionDraftCompletInfo/Complete/" + uid );
            }
            //导出
                ...
                $scope.httpData(aMethed,$scope.credentials.UUID).success(
                    function (data) {
                        if(data.result_code=="S"){
                            ...
                            window.location.href = SRV_URL+"common/RcmFile/downLoad?filePath="+encodeURI(file)+"&fileName="+encodeURI(fileName);
                        }
                    }
            ...
            // 跳转详情页面
            $scope.toNoticeDecisionInfo = function (id, tabIndex, actice) {
                /*href="#/NoticeDecisionDraftDetailView/toSubmit/{{p.BUSINESSID}}/{{'#/NoticeDecisionDraftList/0'|encodeURI}}"*/
                $location.path("/index/NoticeDecisionDraftInfoView/" + actice + "/" + id + "/" + tabIndex);
            };

            // 确定新增项目
            $scope.sure = function (id){
                /*href="#/NoticeDecisionDraftDetail/Create/{{x.UUID}}"*/
                $location.path("/index/NoticeDecisionDraftInfo/Create/" + id );
            };

define(['app', 'Service'], function (app) {
    app.register.controller('PreAuditDetailViewCtrl', ['$stateParams', '$http', '$scope', '$location', 'Upload','CommonService', 'Window', function ($stateParams, $http, $scope, $location, Upload, CommonService, Window) {
        ...
        $scope.getTaskInfoByBusinessId = function (businessId, processKey) {
            $http({
                method: 'post',
                url: SRV_URL + "preAudit/queryTaskInfoByBusinessId.do",
        ...
        $scope.getPreById = function (businessId) {
            $http({
                method: 'post',
                url: SRV_URL + 'preInfo/getPreByID.do',
...
//			}
                ...
                $http({
                    method: 'post',
                    url: SRV_URL + "preInfo/saveReviewInfo.do",
                    data: $.param({
                        "businessId": businessId,
                        "json": JSON.stringify(angular.copy($scope.pre.approveAttachment))
                    })
                }).success(function (data) {
                    if (!data.success) {
                        Window.alert("保存失败");
...
//			});
            ...
            $scope.approve = [{
                submitInfo: {
                    startVar: {
                        ...
                        subject: $scope.pre.apply.projectName + '预评审申请',
        ...
        $scope.taskNodeAudit = function () {
            ...
            //查询评审组长
            $scope.httpData("rcm/Pteam/getTeamLeaderByMemberId", {
                teamMemberId: approveUser.VALUE,
                type: '1'
            }).success(function (data) {
        ...
        $scope.largeAreaFeedbackNodeAudit = function () {
            //子流程中多个节点都调用此页面，流程要根据url中的action决定流程下一步处理人,下一节点名称
            var currentUserName = $scope.credentials.userName;
            var action = $stateParams.action, inputUser, inputUserName, destination, isSkipServiceType;
            var approveUser = $scope.pre.apply.investmentPerson;
            if (approveUser == null || typeof(approveUser) == 'undefined' || approveUser.NAME == null || approveUser.NAME == "") {
                approveUser = $scope.pre.apply.directPerson;
            }
        ...
        $scope.showSubmitModal = function () {
            $scope.save(function () {
                $http({
                    method: 'post',
                    url: SRV_URL + "preAudit/querySingleProcessOptions.do",
                    data: $.param({
                        "businessId": businessId
                    })
                }).success(function (result) {

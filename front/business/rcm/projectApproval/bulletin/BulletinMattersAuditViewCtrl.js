    ...
    app.register.controller('BulletinMattersAuditViewCtrl', ['$http', '$scope', '$location', '$stateParams', '$filter', 'CommonService', 'Window',
        function ($http, $scope, $location, $stateParams, $filter, CommonService, Window, Upload) {
            ...
            $scope.getTaskInfoByBusinessId = function (businessId, processKey) {
                ...
                $http({
                    method: 'post',
                    url: SRV_URL + "bulletinAudit/queryTaskInfoByBusinessId.do",
...
//			$scope.hasWaiting = data.result_data.taskId==null?false:true;
            ...
            $scope.save = function (callback) {
                if ($scope.showController.isLegalLeader) {
                    ...
                    $http({
                        method: 'post',
                        url: SRV_URL + "bulletinInfo/saveLegalLeaderAttachment.do",
                        data: $.param({
                        ...
                        if (data.success) {
                            if (callback) {
                                ...
                                if ($scope.bulletin.legalLeaderAttachment == null || $scope.bulletin.legalLeaderAttachment == "") {
                                    Window.confirm("确认", "您没有上传任何附件，提交后将不能再更改！<br/>是否确认提交！").result.then(function () {
                                        callback();
                                        return;
                                    });
                                }
                    ...
                    $http({
                        method: 'post',
                        url: SRV_URL + "bulletinInfo/saveRiskLeaderAttachment.do",
                        data: $.param({
                            "businessId": queryParamId,
                    ...
                    $http({
                        method: 'post',
                        url: SRV_URL + "bulletinInfo/saveReviewLeaderAttachment.do",
                        data: $.param({
                            "businessId": queryParamId,
            ...
            $scope.initData = function () {
                var url = SRV_URL + "bulletinInfo/queryViewDefaultInfo.do";
                $http({
                    method: 'post',
                    url: url,
            ...
            $scope.uploadReviewLeaderAttachment = function (file, errorFile, idx) {
                if (errorFile && errorFile.length > 0) {
                    ...
                    Upload.upload({
                        url: SRV_URL + 'file/uploadFile.do',
            ...
            $scope.showSubmitModal = function () {
                $scope.save(function () {
                    var url = SRV_URL + "bulletinAudit/querySingleProcessOptions.do";
                    $http({
                        method: 'post',
                        url: url,
                        data: $.param({

...
//	        $scope.showSaveBtn = false;
            ...
            if ($scope.businessId.indexOf("@") > 0) {
                ...
                CommonService.findRelationUser($scope.businessId);
            } else {
                //新流程
...
//			}
                    ...
                    $http({
                        method: 'post',
                        url: SRV_URL + "formalAssessmentInfo/saveLegalReviewInfo.do",
                        data: $.param({
                            "businessId": $scope.businessId,
                            "json": JSON.stringify(angular.copy($scope.pfr.approveLegalAttachment))
                        })
                                    ...
                                    for (var i in commentsList) {
                                        if (commentsList[i].isGrassEdit == '1') {
                                            if (commentsList[i].commentDepartment == null || commentsList[i].commentDepartment == "") {
                                                Window.alert("请根据法律负责人的要求填写反馈内容！");
                                                CommonService.hideMask();
                                                return false;
                                            }
                                        }
...
//			}
                    ...
                    $http({
                        method: 'post',
                        url: SRV_URL + "formalAssessmentInfo/saveReviewInfo.do",
                        data: $.param({
                            "businessId": $scope.businessId,
                            "json": JSON.stringify(angular.copy($scope.pfr.approveAttachment)),
                            "professionalReviewersJson": JSON.stringify($scope.myTaskallocation.professionalReviewers)
                        })
                    }).success(function (data) {
                        if (!data.success) {
                            Window.alert("保存失败");
                            ...
                            if ($scope.showController.isInvestmentManagerBack) {
                                ...
                                for (var i in commentsList) {
                                    if (commentsList[i].isInvestmentManagerBackEdit == '1') {
                                        if (commentsList[i].commentDepartment == null || commentsList[i].commentDepartment == "") {
                                            Window.alert("请根据评审负责人的要求填写反馈内容！");
                                            CommonService.hideMask();
                                            return false;
                                        }
                                    }
                    ...
                    $http({
                        method: 'post',
                        url: SRV_URL + "formalAssessmentInfo/saveMajMemberInfo.do",
                        data: $.param({
                            "businessId": $scope.businessId,
                            "json": JSON.stringify($scope.pfr.approveMajorCommonts)
                        })
                    }).success(function (data) {
                        if (!data.success) {
                            Window.alert("保存失败");
            ...
            $scope.getTaskInfoByBusinessId = function (businessId, processKey) {
                ...
                $http({
                    method: 'post',
                    url: SRV_URL + "formalAssessmentAudit/queryTaskInfoByBusinessId.do",
                    data: $.param(paramObj)
                }).success(function (data) {
...
//    }
            ...
            $scope.showOldSubmitModal = function (type) {
                ...
                $scope.save(function () {
                    if ($scope.oldURL.indexOf("ProjectFormalReviewDetail/Edit") > 0) {
                        ...
                        $scope.approve = [{
                            submitInfo: {
                                startVar: {
                                    processKey: 'formalAssessment',
                                    businessId: params[0],
                                    subject: $scope.pfr.apply.projectName + '正式评审申请',
                                    inputUser: $scope.credentials.UUID
                                },
                        ...
                        if ($stateParams.action == 'View' && params[1]) {
                            $scope.approve[0].submitInfo.taskId = params[3];
                        }
                    } else if ($stateParams.action == 'view1') {
                        ...
                        if (approveUser == null || approveUser.NAME == null || approveUser == "" || approveUser.NAME == "") {
                            var url = SRV_URL + "common/commonMethod/getRoleuserByCode";
                            $.ajax({
                                url: url,
                                type: "POST",
                                data: "5",
                                ...
                                success: function (data) {
                                    if (data.result_code == 'S') {
                                        ...
                                        if (ma) {
                                            nextNode = {
                                                submitInfo: {
                                                    ...
                                                    noticeInfo: {
                                                        ...
                                                        reader: CommonService.removeObjByValue(CommonService.removeDuplicate($scope.relationUsers), ma.VALUE),
                                                        type: '1',
                                                        custText01: '正式评审'
                                                    }
                                                },
                                            ...
                                            Window.alert("评审分配人不存在，请先设置评审分配人！");
                                        }
                                    }
                                }
                            });
                    ...
                    } else if ($stateParams.action == 'view2') {
                        var url = SRV_URL + "common/commonMethod/getRoleuserByCode";
                        $.ajax({
                            url: url,
                            type: "POST",
            ...
            $scope.investmentManagerNodeAudit = function () {
                ...
                if (($stateParams.action == 'Update' || $stateParams.action == 'Edit') && params[1]) {
                    $scope.approve[0].submitInfo.taskId = params[3];
                }
            }
            ...
            $scope.reviewLeaderNodeAudit = function () {
                ...
                if (action == 'approve') {
                    var task = null;
                    var url = SRV_URL + "bpmn/queryTaskById.do";
                    var reqparams = {
                        taskId: params[3]
                    };
                    $.ajax({
                    ...
                    if (task != null && task.description == "isnew") {
                        ...
                        for (var m = 0; m < task.outSequences.length; m++) {
                            ...
                            if (infos.length == 1) {
                                ...
                                //查到待审批人
                                $.ajax({
                                    url: SRV_URL + "formalAudit/queryAuditUsers.do",
                                    type: "POST",
            ...
            $scope.legalNodeAudit = function () {
                ...
                if ($stateParams.action == 'edit1') {
                    ...
                    if (serviceTypes == null || serviceTypes.length == null || serviceTypes.length < 1) {
                        Window.alert("该项目对应的业务类型为空，请检查项目信息是否正确！");
                        return;
                    }
                    ...
                    var approveUser;
                    var url = SRV_URL + "firstlevelLawyer/queryByServiceTypes.do";
                    $.ajax({
                        url: url,
                        type: "POST",
                    ...
                    $scope.approve.approve = [{
                        submitInfo: {
                            ...
                            runtimeVar: {
                                ...
                                legalReviewLeader: legalReviewLeader.VALUE,
                                toTask: '1'
                            },
                            currentTaskVar: {opinion: '请审批'}
                        },
                        showInfo: {
                            destination: '评审负责人及法律负责人审批',
                            approver: approveUser.NAME + '/' + legalReviewLeader.NAME
                        }
                    }, {
                        submitInfo: {
                            taskId: params[3],
            ...
            $scope.showSubmitModal = function () {
                $scope.save(function () {
                    if ($scope.taskMark == null || $scope.taskMark == '') {
                        ...
                        $http({
                            method: 'post',
                            url: SRV_URL + "formalAssessmentAudit/querySingleProcessOptions.do",
                            data: $.param({
                                "businessId": $scope.businessId
                            })
                        }).success(function (result) {
...
//    }
            ...
            $scope.upload2Review = function (file, errorFile, idx) {
                if (errorFile && errorFile.length > 0) {
                    ...
                    Upload.upload({
                        url: SRV_URL + 'common/RcmFile/upload',

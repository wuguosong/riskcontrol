define(['app', 'Service', 'select2'], function (app) {
    app
        .register.controller('formalBiddingInfoViewCtrl', ['$http', '$scope', '$location', '$stateParams', 'BEWG_URL', 'Window',
        function ($http, $scope, $location, $stateParams, BEWG_URL, Window, Upload) {
            console.log('formalBiddingInfoView');
            ...
            $scope.getNoticeDecstionByBusinessId = function(businessId){
                $http({
                    method:'post',
                    url: BEWG_URL.SelectNoticeDecstionById,
            ...
            $scope.upload = function (file,errorFile, idx) {
                if(errorFile && errorFile.length>0){
                    ...
                    Upload.upload({
                        ...
                        Window.alert("文件修改成功！请执行保存操作！否则操作无效！");
                    }, function (resp) {
                        Window.alert(resp.status);
                    }, function (evt) {
                        var progressPercentage = parseInt(100.0 * evt.loaded / evt.total);
            ...
            $scope.cancel = function (){
                $location.path("/index/FormalBiddingInfoList/1");
            };

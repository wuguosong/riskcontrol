define(['app', 'Service', 'select2'], function (app) {
    app
        ...
        function ($http, $scope, $location, $stateParams, $uibModal, BEWG_URL, Window, Upload) {
            ...
            // 申请报告ID
            $scope.complexId = $stateParams.id;
            // 标识进来的方法
            ...
            $scope.saveMarks = function () {
                if ($scope.mark != null && $scope.mark != "") {
                    ...
                    $.ajax({
                        type: 'post',
                        url: BEWG_URL.SaveFormalMarkPfr,
                        data: $.param({
            ...
            $scope.saveMeetingInfo = function (){
                ...
                if ($scope.meetInfo != null && $scope.meetInfo != "") {
                    ...
                    $.ajax({
                        type: 'post',
                        url: BEWG_URL.SaveConferenceInformation,
                        data: $.param({
...
//				Window.alert("此项最高分为"+this.attributes.max.value+"分！");
//				this.value=this.attributes.max.value*1;
//			}
//			if(this.value.indexOf(".")>0){
//				Window.alert("此项只能填写整数！");
//				this.value = 0;
//			}
//		});
                $(".mark").keyup(function () {
            ...
            $scope.getSelectTypeByCode = function (typeCode) {
                $scope.httpData(BEWG_URL.SelecttUserRoleByCode, typeCode).success(function (data) {
                ...
                $http({
                    ...
                    if (data.success) {
                        ...
                        if (method == "so") {
                            ...
                            if (data.result_data) {
                                alertData = "提交成功!";
                                $location.path("/index/FormalBiddingInfoPreview/" + objId + "/2");
                            } else {
                                Window.alert("请确保参会信息已填写完毕!");
                                return false;
                            }
                        }
...
//            	Window.alert("编制人不能为空!");
//            	return false;
//            }

//            if(arrfid[3] == "" || arrfid[9] == ""){
//            	Window.alert("审核人不能为空!");
//            	return false;
//            }
//
            ...
            $scope.uploadReprot = function (file, errorFile, idx) {
                if (errorFile && errorFile.length > 0) {
                    ...
                    Upload.upload({
                        ...
                        Window.alert("文件替换成功！请执行保存操作！否则操作无效！");
                    }, function (resp) {
            ...
            $scope.cancel = function () {
                $location.path("/index/FormalBiddingInfoList/0");
            };
            $scope.changeTemplate = function (obj) {
                var modalInstance = $uibModal.open({
                    animation: true,
                    backdrop: false,
                    controller: 'switchTemplateReminderCtrl',
                    templateUrl: 'business/rcm/projectFillInformation/formalAssessment/biddingInfo/switchTemplateReminder.html',
                ...
                modalInstance.result.then(function () {
                    ...
                    $scope.formalReport.summaryTemplate = angular.copy($scope.initTemplate);
                });
            }
        }]);
    app
        .register.controller('switchTemplateReminderCtrl', ['$http', '$scope', '$uibModal', '$uibModalInstance', 'BEWG_URL', 'Window', 'obj',
    ...
    app
        .register.controller('AddAttenchCtrl', ['$uibModalInstance', '$scope', 'Upload', 'AttachmentList', 'SelectList', 'no',

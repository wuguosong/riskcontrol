define(['app', 'Service', 'select2'], function (app) {
    app
        .register.controller('preBiddingInfoCtrl', ['$http', '$scope', '$location', '$stateParams', 'BEWG_URL', 'Window',
        function ($http, $scope, $location, $stateParams, BEWG_URL, Window, Upload) {
            console.log('preBiddingInfo');
            ...
            $scope.getByBusinessId = function(businessId){
                $http({
                    method:'post',
                    url: BEWG_URL.SelectBiddlingPreById,
                        ...
                        if(method == "so"){
                            ...
                            if(data.result_data){
                               ...
                               /* $location.path("/PreBiddingInfoView/"+businessId+"@view/"+$filter('encodeURI')('#/PreBiddingInfoList/1'));*/
                                $location.path("/index/PreBiddingInfoView/"+businessId+"@view");
                            }else{
                                /*hide_Mask();*/
...
//     	   Window.alert("请选择项目类型!"); return boolean;
//     	}else if(null == jinvestmentAmount || "" == jinvestmentAmount){
//    	    Window.alert("投资金额不能为空!"); return boolean;
//    	}else if(null == jprojectSize || "" == jprojectSize){
//    	   Window.alert("项目规模不能为空!"); return boolean;
//    	}else if(null == rateOfReturn || "" == rateOfReturn){
//    	   Window.alert("投资收益率不能为空!"); return boolean;
//    	}else 
                if(null == fkPsResult || "" == fkPsResult){
                    Window.alert("风控中心评审结论不能为空!"); return boolean;
                }else if(null == fkRiskTip || "" == fkRiskTip){
                    Window.alert("风控重点风险提示不能为空!"); return boolean;
                }else{
                    boolean = true;
                }
            ...
            $scope.cancel = function (){
                $location.path("/index/PreBiddingInfoList/0");
            };

...
public class ReportDataServiceImpl implements IReportDataService {
    ...
    public PageAssistant getProjectList(PageAssistant page, String json) {
        ...
        // 设置表决结果
        JSONObject jsonParams = JSONObject.parseObject(json, JSONObject.class);
        // String userId = this.getMeetingLeaderByCurrentUser(jsonParams.getString("loginUser"));
        // this.setVotingResult(list, userId, jsonParams.getString("meetingLeader"));
    ...
    public String getMeetingLeaderByCurrentUser(String loginUser) {
        ...
        for (Map<String, Object> map : list) {
            String leaders = String.valueOf(map.get("leader"));
            ...
            for (String leader : leadersArray) {
     ...
     * @param list
     */
    private void setVotingResult(List<Map<String, Object>> list, String userId, String defauleValue) {
                    ...
                    if (StringUtils.isNotBlank(userId)) {
                        for (int i = 0; i < leadersArray.length; i++) {
                            if (userId.equals(leadersArray[i])) {
                                idx = i;
                                Map<String, Object> user = userService.getSysUserByID(userId);
                                if (user != null) {
                                    map.put("MEETING_LEADER", user.get("NAME"));
                                }
                                break;
                            }
                        }
                        ...
                        // 全量设置
                        map.put("MEETING_LEADER", map.get("MEETING_LEADERS_C"));
                        map.put("VOTING_RESULT", map.get("VOTING_RESULTS"));
                    }
                }
            }
        }
    ...
    @Override
    public List<Map<String, Object>> filterRoleUser(List<Map<String, Object>> mapList) {
        // 1、获取当前登录用户
        String loginUserId = UserUtil.getCurrentUserUuid();
        // 2、获取风控部门管理员角色
        List<Map<String, Object>> adminList = roleService.queryRoleuserByCode("1");
        if (CollectionUtils.isNotEmpty(adminList)) {
            for (Map<String, Object> admin : adminList) {
                String value = String.valueOf(admin.get("VALUE"));
                if (loginUserId.equals(value)) {
                    return mapList;// 直接返回所有领导列表
                }
            }
        }
        ...
        if (CollectionUtils.isNotEmpty(mapList)) {
            for (Map<String, Object> leader : mapList) {
                String value = String.valueOf(leader.get("VALUE"));
                if (loginUserId.equals(value)) {
                    newMapList.add(leader);
                }
            }
            if (CollectionUtils.isNotEmpty(newMapList)) {
                return newMapList;// 直接返回该领导列表
            }
        }
        // 4、获取秘书角色
        List<Map<String, Object>> mappingList = reportDataMapper.getLeaderSecretaryMappingList();
        if (CollectionUtils.isNotEmpty(mappingList)) {
            for (Map<String, Object> mapping : mappingList) {
                String secretary = String.valueOf(mapping.get("secretary"));
                if (loginUserId.equals(secretary)) {
                    // 获取领导
                    String leaders = String.valueOf(mapping.get("leader"));
                    String[] leadersArray = null;
                    if (leaders.contains(",")) {
                        leadersArray = leaders.split(",");
                    } else {
                    ...
                    // 迭代原始列表
                    for (String leaderStr : leadersArray) {
                        for (Map<String, Object> leader : mapList) {
                            String value = String.valueOf(leader.get("VALUE"));
                            if (leaderStr.equals(value)) {
                                newMapList.add(leader);
                            }
                        }
                    }

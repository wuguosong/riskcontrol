...
public class BpmnService implements IBpmnService {
	...
	public Result deploy(String processKey) {
		...
		this.repositoryService.createDeployment().addInputStream(processKey + ".bpmn", bpmn)
				.addInputStream(processKey + ".png", png).category(processKey).name(processKey).deploy();
	...
	public Result stopProcess(String bpmnType, String businessKey) {
		...
		ProcessInstance processInstance = this.runtimeService.createProcessInstanceQuery()
				.processInstanceBusinessKey(businessKey, bpmnType).singleResult();
		List<HistoricProcessInstance> historyList = this.historyService.createHistoricProcessInstanceQuery()
				.processDefinitionKey(bpmnType).processInstanceBusinessKey(businessKey).list();
		 ...
		 * this.querySequenceFlows(bpmnType, businessKey, userId); return new Result();
		 */
	}

	public List<SequenceFlow> querySequenceFlows(String bpmnType, String businessId, String userId) {
//		userId = "0001N6100000000JJ4DD";
		Task task = this.taskService.createTaskQuery().processDefinitionKey(bpmnType)
				.processInstanceBusinessKey(businessId).taskCandidateOrAssigned(userId).singleResult();
		if (task == null) {
	...
	public Result queryProjectList(String flow) {
		...
		if ("pre".equals(type)) {
			...
			if ("task".equals(task)) {
				...
				for (Map<String, Object> log : preWaitingLogs) {
					if (Util.isNotEmpty(log.get("TASKID"))) {
						...
						if ("1".equals(description.get("isBusinessLeader"))) {
							...
							Map<String, Object> fMongo = this.baseMongo.queryById((String) log.get("BUSINESSID"),
									Constants.RCM_PRE_INFO);
							Map<String, Object> fapply = (Map<String, Object>) fMongo.get("apply");
							// 发送待办信息
							this.reduceWaiting("pre", (String) log.get("BUSINESSID"), newUserId,
									(String) fapply.get("projectName"), (String) log.get("TASKID"));
							// 修改审核日志
				...
				List<Map<String, Object>> queryPertainAreaByOrgPkValue = this.pertainAreaService
						.queryPertainAreaByOrgPkValue((String) reportingUnit.get("KEY"));
				if (queryPertainAreaByOrgPkValue.size() > 0) {
					Map<String, Object> pMap = queryPertainAreaByOrgPkValue.get(0);

					// 判断所有项目是否有当前节点待办，有则处理，无则修改
					List<Map<String, Object>> preWaitingLogs = this.preAuditLogService
							.queryWaitingLogs((String) pMap.get("LEADERID"));
					// 获取当前项目所属大区
					for (Map<String, Object> log : preWaitingLogs) {
						if (Util.isNotEmpty(log.get("TASKID"))) {
							String taskId = (String) log.get("TASKID");
							...
							if (Util.isNotEmpty(documation)) {
								...
								if ("1".equals(description.get("isBusinessArea"))) {
									Map<String, Object> fMongo = this.baseMongo
											.queryById((String) log.get("BUSINESSID"), Constants.RCM_PRE_INFO);
									Map<String, Object> fapply = (Map<String, Object>) fMongo.get("apply");
									// 发送待办信息
									this.reduceWaiting("pre", (String) log.get("BUSINESSID"), newUserId,
											(String) fapply.get("projectName"), (String) log.get("TASKID"));
				...
				for (Map<String, Object> log : queryAuditLogs) {
					if ("9".equals((String) log.get("AUDITSTATUS")) && Util.isNotEmpty(log.get("TASKID"))) {
						...
						String documation = (String) ruTask.get("DESCRIPTION_");
						Document description = Document.parse(documation);
						if ("1".equals(description.get("isInvestmentManager"))
								|| "1".equals(description.get("isInvestmentManagerBack"))) {
							// 发送待办信息
							this.reduceWaiting("pre", businessId, newUserId, (String) apply.get("projectName"),
									(String) log.get("TASKID"));
				...
				for (Map<String, Object> log : queryAuditLogs) {
					if ("9".equals((String) log.get("AUDITSTATUS")) && Util.isNotEmpty(log.get("TASKID"))) {
						...
						if ("1".equals(description.get("isLargeArea"))
								|| "1".equals(description.get("isLargeAreaBack"))) {
							// 发送待办信息
							this.reduceWaiting("pre", businessId, newUserId, (String) apply.get("projectName"),
									(String) log.get("TASKID"));
							// 修改审核日志
...
//				
					...
					for (Map<String, Object> log : preWaitingLogs) {
						if (Util.isNotEmpty(log.get("TASKID"))) {
							...
							if (Util.isNotEmpty(documation)) {
								Document description = Document.parse(documation);
								if ("1".equals(description.get("isBusinessArea"))) {
									Map<String, Object> fMongo = this.baseMongo.queryById(
											(String) log.get("BUSINESSID"), Constants.RCM_FORMALASSESSMENT_INFO);
									Map<String, Object> fapply = (Map<String, Object>) fMongo.get("apply");
									// 发送待办信息
									this.reduceWaiting("pfr", (String) log.get("BUSINESSID"), newUserId,
											(String) fapply.get("projectName"), (String) log.get("TASKID"));
									// 修改审核日志
									this.formalAssessmentAuditLogService.updateWaitingPerson((String) log.get("ID"),
											newUserId);
								}
							}
						}
					...
					if ("9".equals((String) log.get("AUDITSTATUS")) && Util.isNotEmpty(log.get("TASKID"))) {
						...
						if (Util.isNotEmpty(documation)) {
							...
							if ("1".equals(description.get("isInvestmentManager"))
									|| "1".equals(description.get("isInvestmentManagerBack"))) {
								// 发送待办信息
								this.reduceWaiting("pfr", businessId, newUserId, (String) apply.get("projectName"),
										(String) log.get("TASKID"));
								// 修改审核日志
								this.formalAssessmentAuditLogService.updateWaitingPerson((String) log.get("ID"),
										newUserId);
				...
				List<Map<String, Object>> formalWaitingLogs = formalAssessmentAuditLogService
						.queryWaitingLogs(oldUserId);
				for (Map<String, Object> log : formalWaitingLogs) {
					if (Util.isNotEmpty((String) log.get("TASKID"))) {
						...
						if (Util.isNotEmpty(documation)) {
							...
							if ("1".equals(description.get("isSelectLegalLeader"))) {
								...
								Map<String, Object> fMongo = this.baseMongo.queryById(pBusinessId,
										Constants.RCM_FORMALASSESSMENT_INFO);
								Map<String, Object> fApply = (Map<String, Object>) fMongo.get("apply");
								this.sendWaitingToPotal((String) log.get("TASKID"), url, Util.format(Util.now()),
										(String) fApply.get("projectName"), newUserId, "1", "未知");
								;
								// 修改待办日志
								this.formalAssessmentAuditLogService.updateWaitingPerson((String) log.get("ID"),
										newUserId);
			...
			if ("task".equals(task)) {
				...
				for (Map<String, Object> log : queryAuditLogs) {
					if ("9".equals((String) log.get("AUDITSTATUS"))) {
						...
						if (Util.isNotEmpty(documation)) {
							...
							Document description = Document.parse(documation);
							if ("1".equals(description.get("isRiskLeader"))
									|| "1".equals(description.get("isReviewLeader"))) {
								// 发送待办信息
								this.reduceWaiting("bulletin", businessId, newUserId,
										(String) bulletinMongo.get("bulletinName"), (String) log.get("TASKID"));
								// 修改审核日志
						...
						if (Util.isNotEmpty(documation)) {
							...
							if ("1".equals(description.get("isLegalLeader"))) {
								...
								if (Util.isEmpty((String) log.get("OLDUSERID"))) {
									this.bulletinAuditLogService.updateAuditUserIdById(newUserId,
											(String) log.get("ID"));
								}
				...
				data.put("businessId", businessId);
				System.out.print("#################newUserId" + newUserId);
				System.out.print("#################businessId" + businessId);
				this.bulletinInfoService.updatePerson(data);
				// 修改mongo
				Map<String, Object> newUser = userService.queryById(newUserId);
				Map<String, Object> mongoData = new HashMap<String, Object>();
				System.out.print("#################NAME" + (String) newUser.get("NAME"));
				System.out.print("#################VALUE" + (String) newUser.get("UUID"));
				...
				for (Map<String, Object> v : variableList) {
					if ("businessPersonRole".equals(v.get("NAME_"))) {
						...
						List<Map<String, Object>> bulletinWaitingLogs = bulletinAuditLogService
								.queryWaitingLogs(oldUserId);
						...
						for (Map<String, Object> log : bulletinWaitingLogs) {
							...
							if (Util.isNotEmpty((String) log.get("TASKID"))) {
								...
								if ("1".equals(description.get("isBusiness"))) {
									...
									Map<String, Object> bMongo = this.baseMongo.queryById(pBusinessId,
											Constants.RCM_BULLETIN_INFO);
									this.sendWaitingToPotal((String) log.get("TASKID"), url, Util.format(Util.now()),
											(String) bMongo.get("bulletinName"), newUserId, "1", "未知");
									;
		...
		for (Map<String, Object> log : preWaitingLogs) {
			...
			if (Util.isNotEmpty((String) log.get("TASKID"))) {
				...
				if ("1".equals(description.get("isTask"))) {
					...
					this.sendWaitingToPotal((String) log.get("TASKID"), url, Util.format(Util.now()),
							(String) pApply.get("projectName"), newUserId, "1", "未知");
					;
					this.preAuditLogService.updateWaitingPerson((String) log.get("ID"), newUserId);
				}
			}
	...
	@Override
	public void sendWaitingToPotal(String taskId, String url, String createDate, String title, String owner,
				...
				for (Map<String, Object> log : queryAuditLogs) {
					if (Util.isNotEmpty(log.get("TASKID"))) {
						...
						if ("preReview".equals(bpmnType)) {
							...
							String oldurl = "#/FormalAssessmentAuditList/1";
							String out = Util.encodeUrl(oldurl);
							Map<String, Object> formalAssessmentByID = this.FormalAssessmentInfoService
									.getFormalAssessmentByID(businessKey);
							Map<String, Object> formalAssessmentOracle = (Map<String, Object>) formalAssessmentByID
									.get("formalAssessmentOracle");
							...
							String name = (String) this.bulletinInfoService.queryByBusinessId(businessKey)
									.get("BULLETINNAME");
							title = name;
						}

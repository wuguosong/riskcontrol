...
import com.yk.rcm.pre.dao.IPreMarkMapper;
...
@Service
@Transactional
public class PreMarkService implements IPreMarkService {
    @Resource
    private IPreMarkMapper preMarkMapper;

    @Override
    public Map<String, Object> queryMarks(String businessId) {
        return preMarkMapper.queryMarks(businessId);
    }

    @Override
    public void saveOrUpdate(String businessId, String json) {
        HashMap<String, Object> data = new HashMap<String, Object>();
        Map<String, Object> queryMarks = this.preMarkMapper.queryMarks(businessId);
        ...
        if (Util.isEmpty(queryMarks)) {
            ...
            data.put("fileCopy", fileCopy);
            this.preMarkMapper.save(data);
        } else {
            this.preMarkMapper.update(data);
            Boolean heGuiTotalMarkFlag = false;
            Boolean fileTotalMarkFlag = false;
            Boolean heXinTotalMarkFlag = false;
            Map<String, Object> queryMarksForSum = this.preMarkMapper.queryMarks(businessId);
            //计算合规能力总分：流程熟悉度+资料备案
            if (queryMarksForSum.get("FILECOPY") != null && queryMarksForSum.get("FLOWMARK") != null) {
                ...
                this.preMarkMapper.update(map);
                heGuiTotalMarkFlag = true;
            }
            //计算资料质量总分：资料及时性+资料完整性+资料准确定
            if (queryMarksForSum.get("FILETIME") != null && queryMarksForSum.get("FILECONTENT") != null && queryMarksForSum.get("REVIEWFILEACCURACY") != null && queryMarksForSum.get("LEGALFILEACCURACY") != null) {
            ...
            if (queryMarksForSum.get("RISKCONTROL") != null && queryMarksForSum.get("MONEYCALCULATE") != null && queryMarksForSum.get("PLANDESIGN") != null && queryMarksForSum.get("TALKS") != null) {
            ...
            Map<String, Object> queryMarksForAllSum = this.preMarkMapper.queryMarks(businessId);
            //算总数

ctmApp.register.controller('formalDeptWorkList', ['$routeParams','$http','$scope','$location', function ($routeParams,$http,$scope,$location) {
		...
		$http({
			...
			if(result.success){
				...
				if($scope.serviceTypeName){
					$scope.serviceTypeName = $scope.formalAssessmentList[0].mongoDate.apply.serviceType[0].VALUE;
				}else if($scope.areaName){
					$scope.areaName = $scope.formalAssessmentList[0].PERTAINAREANAME;
				}
